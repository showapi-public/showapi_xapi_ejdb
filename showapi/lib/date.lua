...
--date(数字)时，一定要加:tolocal() 。这时的数字是秒
--[[ CONSTANTS ]]--
  local HOURPERDAY  = 24
  local MINPERHOUR  = 60
...
--#end -- not DATE_OBJECT_AFX
  ...
  function date.ts2date(ts14)
  	ts14=ts14..""
  	if string.len(ts14)<14 then return nil end
  	
    local ay = string.sub(ts14,1,4);
    local amon = string.sub(ts14,5,6);
    local ad = string.sub(ts14,7,8);
    local ah = string.sub(ts14,9,10);
    local amin = string.sub(ts14,11,12);
    local as = string.sub(ts14,13,14);
    return date(ay,amon,ad,ah,amin,as)
  end
